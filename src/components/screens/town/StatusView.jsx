      ...
      <div className="flex-shrink-0 p-6 border-b bg-white/50 shadow-sm z-20">
        ...
        <div className="flex flex-col xl:flex-row gap-4 items-start bg-slate-50/50 p-5 rounded-2xl border border-slate-100">
          ...
          <div className="flex flex-col items-center w-full xl:w-64 space-y-3 flex-shrink-0">
            ...
            <div className="text-center w-full">
               ...
               <div className="mt-2 flex justify-center gap-2">
                 <span className="bg-slate-800 text-white px-3 py-1 rounded-full font-black text-xs">{t('LEVEL')} {player.level}</span>
               </div>
            </div>
          </div>
          ...
          <div className="w-full xl:w-72 flex-shrink-0 space-y-1.5">
            <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1 mb-2">{t('EQUIPMENT')}</h3>
          ...
          <div className="flex-1 w-full space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
                <div className="text-[9px] text-slate-400 font-bold mb-1 uppercase">{t('HP')}</div>
              ...
              <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
                <div className="text-[9px] text-slate-400 font-bold mb-1 uppercase">{t('WEIGHT')}</div>
              ...
              <div className="bg-slate-800 p-3 rounded-xl border border-slate-700 shadow-md text-white">
                <div className="text-[9px] text-slate-400 font-bold mb-1 uppercase">{t('ATTACK')}</div>
                <div className="text-xl font-mono font-bold leading-none">{battle.atk}</div>
              </div>
              <div className="bg-slate-800 p-3 rounded-xl border border-slate-700 shadow-md text-white">
                <div className="text-[9px] text-slate-400 font-bold mb-1 uppercase">{t('DEFENSE')}</div>
              ...
              <div className="grid grid-cols-5 xl:grid-cols-2 gap-x-6 gap-y-2">
                {['STR', 'VIT', 'DEX', 'AGI', 'LUK'].map(stat => (
                  <div key={stat} className="flex flex-col xl:flex-row xl:justify-between border-b border-slate-50 py-1">
                    <span className="text-[10px] text-slate-400 font-black">{t(stat)}</span>
                ...
                >
                  ...
                  <div className="flex-1 grid grid-cols-2 gap-x-4 gap-y-2 bg-slate-100/50 p-3 rounded-xl border border-slate-100">
                    <div className="col-span-2 flex justify-between items-center border-b-2 border-slate-200 pb-2 mb-1">
                      <span className="text-xs font-black text-green-700">{t('HP')} {char.stats.hp}</span>
                      {char.plusCount > 0 && (
                    ...
                    {[
                      { key: 'STR', val: char.stats.str, color: 'text-red-600' },
                      { key: 'VIT', val: char.stats.vit, color: 'text-orange-600' },
                      { key: 'DEX', val: char.stats.dex, color: 'text-purple-600' },
                      { key: 'AGI', val: char.stats.agi, color: 'text-blue-600' },
                      { key: 'LUK', val: char.stats.luk, color: 'text-yellow-700' }
                    ].map((s) => (
                      <div key={s.key} className="flex justify-between items-center border-b border-slate-200/50 pb-1">
                        <span className={`text-[10px] font-black ${s.color}`}>{t(s.key)}</span>
